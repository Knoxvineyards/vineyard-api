...
function parseEcowittData(data) {
  ...
  // Temperature (from WN35 or other temp sensor)
  ...
  // Soil Moisture Sensors (WH51L)
  // soilmoisture1, soilmoisture2, etc. - values are 0-100%
  parsed.soilMoisture1 = data.soilmoisture1 ? parseFloat(data.soilmoisture1) : null;
  parsed.soilMoisture2 = data.soilmoisture2 ? parseFloat(data.soilmoisture2) : null;

  // Leaf Wetness (WN35)
  // leafwetness_ch1 - values typically 0-100%
  parsed.leafWetness = data.leafwetness_ch1 ? parseFloat(data.leafwetness_ch1) : null;

  // Additional useful fields
  parsed.stationtype = data.stationtype || 'Unknown';
  parsed.passkey = data.PASSKEY || data.passkey || 'Unknown';
...
app.post('/api/ecowitt', (req, res) => {
  try {
    console.log('Received Ecowitt data:', req.body);
    ...
    // Ecowitt expects a success response
